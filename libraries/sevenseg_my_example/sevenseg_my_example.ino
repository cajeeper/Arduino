/*************************************************** 
  This is a library for our I2C LED Backpacks

  Designed specifically to work with the Adafruit LED 7-Segment backpacks 
  ----> http://www.adafruit.com/products/881
  ----> http://www.adafruit.com/products/880
  ----> http://www.adafruit.com/products/879
  ----> http://www.adafruit.com/products/878

  These displays use I2C to communicate, 2 pins are required to 
  interface. There are multiple selectable I2C addresses. For backpacks
  with 2 Address Select pins: 0x70, 0x71, 0x72 or 0x73. For backpacks
  with 3 Address Select pins: 0x70 thru 0x77

  Adafruit invests time and resources providing this open source code, 
  please support Adafruit and open-source hardware by purchasing 
  products from Adafruit!

  Written by Limor Fried/Ladyada for Adafruit Industries.  
  BSD license, all text above must be included in any redistribution
 ****************************************************/
 
#include <Wire.h>
#include "Adafruit_LEDBackpack.h"
#include "Adafruit_GFX.h"

Adafruit_7segment matrix = Adafruit_7segment();
Adafruit_7segment matrix2 = Adafruit_7segment();
Adafruit_7segment matrix3 = Adafruit_7segment();
//Adafruit_7segment matrix4 = Adafruit_7segment();

void setup() {
  Serial.begin(9600);
  Serial.println("7 Segment Backpack Test");

  matrix.begin(0x70);
  matrix2.begin(0x71);
  matrix3.begin(0x72);
//  matrix4.begin(0x73);
  
}

void loop() {
  // try to print a number thats too long
  matrix.print(10000, DEC);
  matrix.writeDisplay();
  matrix2.print(10000, DEC);
  matrix2.writeDisplay();
  matrix3.print(10000, DEC);
  matrix3.writeDisplay();
//  matrix4.print(10000, DEC);
//  matrix4.writeDisplay();
  delay(1000);
  matrix.print(1, DEC);
  matrix.writeDisplay();
  matrix2.print(2, DEC);
  matrix2.writeDisplay();
  matrix3.print(3, DEC);
  matrix3.writeDisplay();
//  matrix4.print(4, DEC);
//  matrix4.writeDisplay();
  delay(1000);
  matrix.print(10000, DEC);
  matrix.writeDisplay();
  matrix2.print(10000, DEC);
  matrix2.writeDisplay();
  matrix3.print(10000, DEC);
  matrix3.writeDisplay();
//  matrix4.print(10000, DEC);
//  matrix4.writeDisplay();
  delay(1000);
  matrix.print(1, DEC);
  matrix.writeDisplay();
  matrix2.print(2, DEC);
  matrix2.writeDisplay();
  matrix3.print(3, DEC);
  matrix3.writeDisplay();
//  matrix4.print(4, DEC);
//  matrix4.writeDisplay();
  delay(4000);
  matrix.print(8888, DEC);
  matrix.writeDisplay();
  matrix2.print(8888, DEC);
  matrix2.writeDisplay();
  matrix3.print(8888, DEC);
  matrix3.writeDisplay();
//  matrix4.print(8888, DEC);
//  matrix4.writeDisplay();
  delay(1000);
  matrix.print(10000, DEC);
  matrix.writeDisplay();
  matrix2.print(10000, DEC);
  matrix2.writeDisplay();
  matrix3.print(10000, DEC);
  matrix3.writeDisplay();
//  matrix4.print(10000, DEC);
//  matrix4.writeDisplay();
  delay(1000);
  matrix.clear();
  matrix.writeDisplay();
  matrix2.clear();
  matrix2.writeDisplay();
  matrix3.clear();
  matrix3.writeDisplay();
//  matrix4.clear();
//  matrix4.writeDisplay();
  delay(800);
  
  
  matrix.writeDigitNum(0, 8);
  matrix.writeDisplay();
  delay(100);
  matrix.clear();
  matrix.writeDigitNum(1, 8);
  matrix.writeDisplay();
  delay(100);
  matrix.clear();
  matrix.writeDigitNum(3, 8);
  matrix.writeDisplay();
  delay(100);
  matrix.clear();
  matrix.writeDigitNum(4, 8);
  matrix.writeDisplay();
  delay(100);
  matrix.clear();
  matrix.writeDisplay();
  matrix2.writeDigitNum(0, 8);
  matrix2.writeDisplay();
  delay(100);
  matrix2.clear();
  matrix2.writeDigitNum(1, 8);
  matrix2.writeDisplay();
  delay(100);
  matrix2.clear();
  matrix2.writeDigitNum(3, 8);
  matrix2.writeDisplay();
  delay(100);
  matrix2.clear();
  matrix2.writeDigitNum(4, 8);
  matrix2.writeDisplay();
  delay(100);
  matrix2.clear();
  matrix2.writeDisplay();
  matrix3.writeDigitNum(0, 8);
  matrix3.writeDisplay();
  delay(100);
  matrix3.clear();
  matrix3.writeDigitNum(1, 8);
  matrix3.writeDisplay();
  delay(100);
  matrix3.clear();
  matrix3.writeDigitNum(3, 8);
  matrix3.writeDisplay();
  delay(100);
  matrix3.clear();
  matrix3.writeDigitNum(4, 8);
  matrix3.writeDisplay();
  delay(100);
  matrix3.clear();
  matrix3.writeDisplay();
//  matrix4.writeDigitNum(0, 8);
//  matrix4.writeDisplay();
//  delay(100);
//  matrix4.clear();
//  matrix4.writeDigitNum(1, 8);
//  matrix4.writeDisplay();
//  delay(100);
//  matrix4.clear();
//  matrix4.writeDigitNum(3, 8);
//  matrix4.writeDisplay();
//  delay(100);
//  matrix4.clear();
//  matrix4.writeDigitNum(4, 8);
//  matrix4.writeDisplay();
//  delay(100);
//  matrix4.clear();
//  matrix4.writeDigitNum(3, 8);
//  matrix4.writeDisplay();
//  delay(100);
/*  matrix4.clear();
  matrix4.writeDigitNum(1, 8);
  matrix4.writeDisplay();
  delay(100);
  matrix4.clear();
  matrix4.writeDigitNum(0, 8);
  matrix4.writeDisplay();
  delay(100);  
  matrix4.clear();
  matrix4.writeDisplay();
*/  matrix3.writeDigitNum(4, 8);
  matrix3.writeDisplay();
  delay(100);
  matrix3.clear();
  matrix3.writeDigitNum(3, 8);
  matrix3.writeDisplay();
  delay(100);
  matrix3.clear();
  matrix3.writeDigitNum(1, 8);
  matrix3.writeDisplay();
  delay(100);
  matrix3.clear();
  matrix3.writeDigitNum(0, 8);
  matrix3.writeDisplay();
  delay(100);  
  matrix3.clear();
  matrix3.writeDisplay();
  matrix2.writeDigitNum(4, 8);
  matrix2.writeDisplay();
  delay(100);
  matrix2.clear();
  matrix2.writeDigitNum(3, 8);
  matrix2.writeDisplay();
  delay(100);
  matrix2.clear();
  matrix2.writeDigitNum(1, 8);
  matrix2.writeDisplay();
  delay(100);
  matrix2.clear();
  matrix2.writeDigitNum(0, 8);
  matrix2.writeDisplay();
  delay(100);  
  matrix2.clear();
  matrix2.writeDisplay();
  matrix.writeDigitNum(4, 8);
  matrix.writeDisplay();
  delay(100);
  matrix.clear();
  matrix.writeDigitNum(3, 8);
  matrix.writeDisplay();
  delay(100);
  matrix.clear();
  matrix.writeDigitNum(1, 8);
  matrix.writeDisplay();
  delay(100);
  matrix.clear();
  matrix.writeDigitNum(0, 8);
  matrix.writeDisplay();
  delay(100);  
  matrix.clear();
  matrix.writeDisplay();
  delay(1000);
  

  matrix.writeDigitNum(0, 8);
  matrix.writeDisplay();
  delay(25);
  matrix.clear();
  matrix.writeDigitNum(1, 8);
  matrix.writeDisplay();
  delay(25);
  matrix.clear();
  matrix.writeDigitNum(3, 8);
  matrix.writeDisplay();
  delay(25);
  matrix.clear();
  matrix.writeDigitNum(4, 8);
  matrix.writeDisplay();
  delay(25);
  matrix.clear();
  matrix.writeDisplay();
  matrix2.writeDigitNum(0, 8);
  matrix2.writeDisplay();
  delay(25);
  matrix2.clear();
  matrix2.writeDigitNum(1, 8);
  matrix2.writeDisplay();
  delay(25);
  matrix2.clear();
  matrix2.writeDigitNum(3, 8);
  matrix2.writeDisplay();
  delay(25);
  matrix2.clear();
  matrix2.writeDigitNum(4, 8);
  matrix2.writeDisplay();
  delay(25);
  matrix2.clear();
  matrix2.writeDisplay();
  matrix3.writeDigitNum(0, 8);
  matrix3.writeDisplay();
  delay(25);
  matrix3.clear();
  matrix3.writeDigitNum(1, 8);
  matrix3.writeDisplay();
  delay(25);
  matrix3.clear();
  matrix3.writeDigitNum(3, 8);
  matrix3.writeDisplay();
  delay(25);
  matrix3.clear();
  matrix3.writeDigitNum(4, 8);
  matrix3.writeDisplay();
  delay(25);
  matrix3.clear();
  matrix3.writeDisplay();
/*  matrix4.writeDigitNum(0, 8);
  matrix4.writeDisplay();
  delay(25);
  matrix4.clear();
  matrix4.writeDigitNum(1, 8);
  matrix4.writeDisplay();
  delay(25);
  matrix4.clear();
  matrix4.writeDigitNum(3, 8);
  matrix4.writeDisplay();
  delay(25);
  matrix4.clear();
  matrix4.writeDigitNum(4, 8);
  matrix4.writeDisplay();
  delay(25);
  matrix4.clear();
  matrix4.writeDigitNum(3, 8);
  matrix4.writeDisplay();
  delay(25);
  matrix4.clear();
  matrix4.writeDigitNum(1, 8);
  matrix4.writeDisplay();
  delay(25);
  matrix4.clear();
  matrix4.writeDigitNum(0, 8);
  matrix4.writeDisplay();
  delay(25);  
  matrix4.clear();
  matrix4.writeDisplay();
*/  matrix3.writeDigitNum(4, 8);
  matrix3.writeDisplay();
  delay(25);
  matrix3.clear();
  matrix3.writeDigitNum(3, 8);
  matrix3.writeDisplay();
  delay(25);
  matrix3.clear();
  matrix3.writeDigitNum(1, 8);
  matrix3.writeDisplay();
  delay(25);
  matrix3.clear();
  matrix3.writeDigitNum(0, 8);
  matrix3.writeDisplay();
  delay(25);  
  matrix3.clear();
  matrix3.writeDisplay();
  matrix2.writeDigitNum(4, 8);
  matrix2.writeDisplay();
  delay(25);
  matrix2.clear();
  matrix2.writeDigitNum(3, 8);
  matrix2.writeDisplay();
  delay(25);
  matrix2.clear();
  matrix2.writeDigitNum(1, 8);
  matrix2.writeDisplay();
  delay(25);
  matrix2.clear();
  matrix2.writeDigitNum(0, 8);
  matrix2.writeDisplay();
  delay(25);  
  matrix2.clear();
  matrix2.writeDisplay();
  matrix.writeDigitNum(4, 8);
  matrix.writeDisplay();
  delay(25);
  matrix.clear();
  matrix.writeDigitNum(3, 8);
  matrix.writeDisplay();
  delay(25);
  matrix.clear();
  matrix.writeDigitNum(1, 8);
  matrix.writeDisplay();
  delay(25);
  matrix.clear();
  matrix.writeDigitNum(0, 8);
  matrix.writeDisplay();
  delay(25);  
  matrix.clear();
  matrix.writeDisplay();

  matrix.writeDigitNum(0, 8);
  matrix.writeDisplay();
  delay(25);
  matrix.clear();
  matrix.writeDigitNum(1, 8);
  matrix.writeDisplay();
  delay(25);
  matrix.clear();
  matrix.writeDigitNum(3, 8);
  matrix.writeDisplay();
  delay(25);
  matrix.clear();
  matrix.writeDigitNum(4, 8);
  matrix.writeDisplay();
  delay(25);
  matrix.clear();
  matrix.writeDisplay();
  matrix2.writeDigitNum(0, 8);
  matrix2.writeDisplay();
  delay(25);
  matrix2.clear();
  matrix2.writeDigitNum(1, 8);
  matrix2.writeDisplay();
  delay(25);
  matrix2.clear();
  matrix2.writeDigitNum(3, 8);
  matrix2.writeDisplay();
  delay(25);
  matrix2.clear();
  matrix2.writeDigitNum(4, 8);
  matrix2.writeDisplay();
  delay(25);
  matrix2.clear();
  matrix2.writeDisplay();
  matrix3.writeDigitNum(0, 8);
  matrix3.writeDisplay();
  delay(25);
  matrix3.clear();
  matrix3.writeDigitNum(1, 8);
  matrix3.writeDisplay();
  delay(25);
  matrix3.clear();
  matrix3.writeDigitNum(3, 8);
  matrix3.writeDisplay();
  delay(25);
  matrix3.clear();
  matrix3.writeDigitNum(4, 8);
  matrix3.writeDisplay();
  delay(25);
  matrix3.clear();
  matrix3.writeDisplay();
/*  matrix4.writeDigitNum(0, 8);
  matrix4.writeDisplay();
  delay(25);
  matrix4.clear();
  matrix4.writeDigitNum(1, 8);
  matrix4.writeDisplay();
  delay(25);
  matrix4.clear();
  matrix4.writeDigitNum(3, 8);
  matrix4.writeDisplay();
  delay(25);
  matrix4.clear();
  matrix4.writeDigitNum(4, 8);
  matrix4.writeDisplay();
  delay(25);
  matrix4.clear();
  matrix4.writeDigitNum(3, 8);
  matrix4.writeDisplay();
  delay(25);
  matrix4.clear();
  matrix4.writeDigitNum(1, 8);
  matrix4.writeDisplay();
  delay(25);
  matrix4.clear();
  matrix4.writeDigitNum(0, 8);
  matrix4.writeDisplay();
  delay(25);  
  matrix4.clear();
  matrix4.writeDisplay();
*/  matrix3.writeDigitNum(4, 8);
  matrix3.writeDisplay();
  delay(25);
  matrix3.clear();
  matrix3.writeDigitNum(3, 8);
  matrix3.writeDisplay();
  delay(25);
  matrix3.clear();
  matrix3.writeDigitNum(1, 8);
  matrix3.writeDisplay();
  delay(25);
  matrix3.clear();
  matrix3.writeDigitNum(0, 8);
  matrix3.writeDisplay();
  delay(25);  
  matrix3.clear();
  matrix3.writeDisplay();
  matrix2.writeDigitNum(4, 8);
  matrix2.writeDisplay();
  delay(25);
  matrix2.clear();
  matrix2.writeDigitNum(3, 8);
  matrix2.writeDisplay();
  delay(25);
  matrix2.clear();
  matrix2.writeDigitNum(1, 8);
  matrix2.writeDisplay();
  delay(25);
  matrix2.clear();
  matrix2.writeDigitNum(0, 8);
  matrix2.writeDisplay();
  delay(25);  
  matrix2.clear();
  matrix2.writeDisplay();
  matrix.writeDigitNum(4, 8);
  matrix.writeDisplay();
  delay(25);
  matrix.clear();
  matrix.writeDigitNum(3, 8);
  matrix.writeDisplay();
  delay(25);
  matrix.clear();
  matrix.writeDigitNum(1, 8);
  matrix.writeDisplay();
  delay(25);
  matrix.clear();
  matrix.writeDigitNum(0, 8);
  matrix.writeDisplay();
  delay(25);  
  matrix.clear();
  matrix.writeDisplay();
  delay(3000);
  
  // print a hex number
  //matrix.print(0xBEEF, HEX);
  //matrix.writeDisplay();
  //delay(500);

  // print a floating point 
  /* this isnt working yet, still hacking on it!
  matrix.print(12.34);
  matrix.writeDisplay();
  delay(500);
  */
  
  // print with print/println
  /*
  for (uint16_t counter = 0; counter < 100; counter++) {
    matrix.println(counter);
    matrix.writeDisplay();
    delay(10);
  }
  */

  /*// method #2 - draw each digit
  uint16_t blinkcounter = 0;
  boolean drawDots = false;
  for (uint16_t counter = 0; counter < 100; counter ++) {
    matrix.writeDigitNum(0, (counter / 1000), drawDots);
    matrix.writeDigitNum(1, (counter / 100) % 10, drawDots);  
    matrix.drawColon(drawDots);
    matrix.writeDigitNum(3, (counter / 10) % 10, drawDots);
    matrix.writeDigitNum(4, counter % 10, drawDots);
   
    blinkcounter+=50;
    if (blinkcounter < 500) {
      drawDots = false;
    } else if (blinkcounter < 1000) {
      drawDots = true;
    } else {
      blinkcounter = 0;
    }
    matrix.writeDisplay();
    delay(10);
  }
  */
}
